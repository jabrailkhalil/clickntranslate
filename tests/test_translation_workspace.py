...
def test_stacked_editors_stay_inside_their_frames_at_default_scale(app, workspace):
    ...
    try:
        workspace.resize(500, 620)
        for _ in range(5):
            app.processEvents()
        assert workspace._panel_orientation is False
        for panel, editor in ((workspace.source_panel, workspace.source_edit),
                              (workspace.result_panel, workspace.text_edit)):
            margins = panel.layout().contentsMargins()
            interior = panel.rect().adjusted(margins.left(), margins.top(),
                                             -margins.right(), -margins.bottom())
            assert interior.contains(editor.geometry()), (interior, editor.geometry())
        assert workspace.height() >= workspace.minimumSizeHint().height()
    finally:
        workspace.close()
        app.removeEventFilter(appearance)
        del app._dialog_appearance

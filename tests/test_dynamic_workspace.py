...
def isolated(app, monkeypatch, tmp_path):
    # These workflow scenarios use explicit logical source/output rectangles.
    # Keep their desktop stable when QT_SCALE_FACTOR changes the offscreen
    # plugin's default 800x600 screen into e.g. 400x300 logical units.
    desktop = QtCore.QRect(0, 0, 1280, 900)
    monkeypatch.setattr(QtGui.QScreen, 'geometry', lambda self: QtCore.QRect(desktop))
    monkeypatch.setattr(QtGui.QScreen, 'availableGeometry', lambda self: QtCore.QRect(desktop))
...
def test_output_controls_paint_themed_background_on_translucent_surfaces(isolated, monkeypatch, theme, background, windows):
    ...
    try:
        ...
        # Sample the frame margin, not the now permanently visible Stop button.
        color = image.pixelColor(round(2 * ratio), round(controls.height() * .5 * ratio))
...
def test_output_controls_large_dpi_fit_screen_without_scrolling_and_keep_stop_visible(isolated, app):
    ...
    try:
        controls = overlay.controls
        controls.gear.click()
        controls.set_error('Long diagnostic message. ' * 40)
        app.processEvents()
        controls._resize_panel()
        app.processEvents()

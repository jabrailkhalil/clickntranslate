...
for (const [lang, t] of Object.entries(content)) {
  ...
  const languageLinks = Object.entries(languages)
    .map(
      ([code, label]) =>
        `<a href="${prefix}${code === "en" ? "./" : code + "/"}" lang="${code}" hreflang="${code}" data-language-choice="${code}" ${code === lang ? 'aria-current="page"' : ""}>${label}<span aria-hidden="true">${code === lang ? "✓" : ""}</span></a>`,
    )
    .join("");
...
<html lang="${lang}"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(t.title)}</title><meta name="description" content="${esc(t.description)}"><meta name="theme-color" content="#f7f6f2">
...
<link rel="icon" href="${asset("icon.png")}"><link rel="stylesheet" href="${asset("fonts/fonts.css")}"><link rel="stylesheet" href="${prefix}styles.css?v=20260925-3">
...
<section class="section companion wrap" id="companion"><div class="feature-row"><div class="companion-picture"><div class="theme-switch" role="group" aria-label="${t.companionNote}">${t.themes.map((label, i) => `<button type="button" aria-pressed="${i === 0}" data-theme-preview="${i === 0 ? "light" : "dark"}">${label}</button>`).join("")}</div><img id="companion-preview" src="${asset("mascot-light.png")}" data-light="${asset("mascot-light.png")}" data-dark="${asset("mascot-dark.png")}" width="831" height="559" alt="${t.companionAlt}" loading="lazy"></div><div class="feature-copy"><img class="companion-character" src="${asset("companion.png")}" width="72" height="72" alt="" loading="lazy"><p class="eyebrow">${t.companionKicker}</p><h2>${t.companionTitle}</h2><p>${t.companionText}</p><p class="caption">${t.companionNote}</p></div></div></section>

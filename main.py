...
class DarkThemeApp(QMainWindow):
    def __init__(self):
        ...
        self.hotkey_thread = HotkeyListenerThread(ocr_hotkey, self.launch_ocr, hotkey_id=1)
        ...
        if copy_hotkey:
            self.copy_hotkey_thread = HotkeyListenerThread(copy_hotkey, self.launch_copy, hotkey_id=2)
            self.copy_hotkey_thread.start()
        translate_hotkey = self.config.get("translate_hotkey", "")
        if translate_hotkey:
            self.translate_hotkey_thread = HotkeyListenerThread(translate_hotkey, self.launch_translate, hotkey_id=3)
    ...
    def show_help_dialog(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Помощь" if lang == "ru" else "Help")
        ...
        # Текст помощи
        if lang == "ru":
            help_text = """<h3 style='color:#7A5FA1;'>🚀 Как пользоваться</h3>
<p><b>1.</b> Нажмите горячую клавишу (по умолчанию <b>Ctrl+Shift+C</b> для копирования или <b>Ctrl+Shift+R</b> для перевода)</p>
<p><b>2.</b> Выделите область экрана с текстом мышкой</p>
<p><b>3.</b> Выберите язык текста на экране (флаг в углу)</p>
<p><b>4.</b> Текст будет распознан и скопирован в буфер обмена (или переведён)</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>Переводчики</h3>
<p><b>Google</b> — быстрый и точный. <span style='color:green;'>✓ Рекомендуется</span></p>
<p><b>Argos</b> — офлайн, без интернета, приватный.</p>
<p><b>MyMemory</b> — бесплатный API. Лимит ~5000 симв/день.</p>
<p><b>Lingva / LibreTranslate</b> — альтернативные публичные серверы.</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>OCR движки</h3>
<p><b>Windows</b> — встроенный. <span style='color:green;'>✓ Рекомендуется</span></p>
<p><b>Tesseract</b> — офлайн, высокая точность.</p>
<p><b>RapidOCR</b> — сверхбыстрый (ONNX).</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>⚡ Портативность</h3>
<p>Программа <b>полностью портативна</b> — не требует установки.</p>
<p><b>Автозапуск:</b> Если отключить через Диспетчер задач — включите обратно там же.</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>Горячие клавиши</h3>
<p>Если хоткей не работает — возможно он занят другой программой. Попробуйте другую комбинацию в настройках.</p>"""
        else:
            help_text = """<h3 style='color:#7A5FA1;'>🚀 How to Use</h3>
<p><b>1.</b> Press a hotkey (default: <b>Ctrl+Shift+C</b> to copy or <b>Ctrl+Shift+R</b> to translate)</p>
<p><b>2.</b> Select an area on screen with text using your mouse</p>
<p><b>3.</b> Choose the text language (flag in the corner)</p>
<p><b>4.</b> Text will be recognized and copied to clipboard (or translated)</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>Translators</h3>
<p><b>Google</b> — fast and accurate. <span style='color:green;'>✓ Recommended</span></p>
<p><b>Argos</b> — offline, no internet, private.</p>
<p><b>MyMemory</b> — free API. Limit ~5000 chars/day.</p>
<p><b>Lingva / LibreTranslate</b> — alternative public servers.</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>OCR Engines</h3>
<p><b>Windows</b> — built-in. <span style='color:green;'>✓ Recommended</span></p>
<p><b>Tesseract</b> — offline, high accuracy.</p>
<p><b>RapidOCR</b> — super-fast (ONNX).</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>⚡ Portable App</h3>
<p>This app is <b>fully portable</b> — no installation required.</p>
<p><b>Autostart:</b> If disabled via Task Manager — re-enable it there.</p>

<h3 style='color:#7A5FA1; margin-top:15px;'>Hotkeys</h3>
<p>If a hotkey doesn't work — it may be taken by another program. Try a different combination in settings.</p>"""
...
if __name__ == "__main__":
    ...
    if getattr(sys, 'frozen', False):
        def bring_existing_to_front():
            """Найти и активировать окно уже запущенного процесса."""
            this_pid = os.getpid()
            current_exe = sys.executable.lower()
                ...
                try:
                    if p.info['exe'] and p.info['exe'].lower() == current_exe:
                        if p.info['pid'] != this_pid:
                            ...
                            # Ищем окно по PID процесса через EnumWindows
                            found_hwnd = [None]
                            ...
                            def enum_callback(hwnd, lparam):
                                ...
                                if pid.value == target_pid:
                                    # Проверяем, что это главное окно (имеет заголовок)
                                    length = ctypes.windll.user32.GetWindowTextLengthW(hwnd)
                                    if length > 0:
                                        found_hwnd[0] = hwnd
                                        return False  # Прекратить перечисление
